...
to the global defaults. Mocks the model boundary (`_chat`) and asserts each
...
def test_per_lane_knobs_route_to_each_role(monkeypatch):
    ...
    knobs = {
        "answer": {"temperature": 0.7, "repeat_penalty": 1.3, "dry": 0.5},
        "review": {"temperature": 0.9, "repeat_penalty": 1.1, "dry": 0.2},
    ...
    assert all(
        c["temperature"] == 0.7 and c["repeat_penalty"] == 1.3 and c["dry"] == 0.5
        for c in synth
    ), seen
    ...
    assert all(
        c["temperature"] == 0.9 and c["repeat_penalty"] == 1.1 and c["dry"] == 0.2
        for c in val
    ), seen
...
def test_unset_role_knobs_keep_defaults(monkeypatch):
    ...
    for c in synth:
        ...
        assert c["repeat_penalty"] == config.SYNTH_REPEAT_PENALTY, c
        assert c["dry"] == config.SYNTH_DRY_MULTIPLIER, c
