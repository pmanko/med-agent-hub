...
class QueryStoreClient:
    ...
    async def get_patient_chart(self, patient_uuid: str, *, page_size: int = 500) -> list[dict[str, Any]]:
        ...
        Pages until the server's ``totalCount`` is reached and requires unique record identities.
        The endpoint is a thin adapter over querystore's existing ``getPatientChart`` behavior; index
        readiness and repair are separate operational concerns.
        Raises
